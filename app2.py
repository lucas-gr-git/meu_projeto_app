...
# --- BUSCA DIVIDENDOS NA B3 ---
def buscar_dividendos_b3(ticker):
    try:
        url = f"https://sistemaswebb3-listados.b3.com.br/listedCompaniesProxy/CompanyCall/GetListedCashDividends/{ticker}"
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req, timeout=10) as res:
            data = json.loads(res.read().decode('utf-8'))
        
        eventos = data.get('cashDividends', [])
        if not eventos:
            return None, None, None
        
        # Pega o mais recente
        ultimo = eventos[0]
        tipo        = ultimo.get('label', 'Dividendo')
        data_com    = ultimo.get('lastDatePrior', None)
        pagamento   = ultimo.get('paymentDate', None)
        valor       = ultimo.get('value', None)
        
        if data_com:
            data_com = datetime.strptime(data_com[:10], '%Y-%m-%d').strftime('%d/%m/%Y')
        if pagamento:
            pagamento = datetime.strptime(pagamento[:10], '%Y-%m-%d').strftime('%d/%m/%Y')
        if valor:
            valor = f"R$ {float(valor):,.4f}".replace(',', 'X').replace('.', ',').replace('X', '.')

        return tipo, data_com, pagamento, valor
    except:
        return None, None, None, None

# --- BUSCA PRÓXIMO BALANÇO NA CVM ---
def buscar_proximo_balanco_cvm(ticker):
    try:
        # Busca ITRs mais recentes na CVM (arquivos trimestrais)
        url = "https://dados.cvm.gov.br/dados/CIA_ABERTA/DOC/ITR/META/ITR_CIA_ABERTA.csv"
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req, timeout=10) as res:
            df_cvm = pd.read_csv(res, sep=';', encoding='latin1')
        
        # Filtra pelo ticker (campo DENOM_CIA contém o nome)
        ticker_base = ticker[:4].upper()
        df_filtrado = df_cvm[df_cvm['CD_CVM'].astype(str).str.contains(ticker_base, na=False) | 
                             df_cvm.get('DENOM_CIA', pd.Series()).astype(str).str.upper().str.contains(ticker_base, na=False)]
        
        if not df_filtrado.empty:
            ultima = df_filtrado.sort_values('DT_REFER', ascending=False).iloc[0]
            return ultima.get('DT_REFER', None)
        return None
    except:
        return None

with st.spinner("Buscando eventos na B3 e CVM..."):
    tipo_div, data_com_b3, data_pgto, valor_div = buscar_dividendos_b3(codigo_limpo)
...
with c_ev1:
    ...
    <div style="border:1px solid #333; border-radius:8px; background:#1e1e1e; overflow:hidden;">
        ...
        <div style="padding:0 15px 10px; color:#888; font-size:12px;">Fonte: B3 Oficial</div>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("#### 📋 Histórico de Proventos (B3)")

def buscar_historico_proventos_b3(ticker):
    try:
        url = f"https://sistemaswebb3-listados.b3.com.br/listedCompaniesProxy/CompanyCall/GetListedCashDividends/{ticker}"
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req, timeout=10) as res:
        ...
        if not eventos:
            return pd.DataFrame()
        df_prov = pd.DataFrame(eventos)[['label', 'lastDatePrior', 'paymentDate', 'value']]
        df_prov.columns = ['Tipo', 'Data Com', 'Pagamento', 'Valor (R$)']
        df_prov['Data Com'] = pd.to_datetime(df_prov['Data Com']).dt.strftime('%d/%m/%Y')
        df_prov['Pagamento'] = pd.to_datetime(df_prov['Pagamento']).dt.strftime('%d/%m/%Y')
        df_prov['Valor (R$)'] = df_prov['Valor (R$)'].apply(lambda x: f"R$ {float(x):,.4f}".replace(',','X').replace('.', ',').replace('X','.'))
        return df_prov
    except:
        return pd.DataFrame()

df_proventos = buscar_historico_proventos_b3(codigo_limpo)
if not df_proventos.empty:
    st.dataframe(df_proventos, use_container_width=True, hide_index=True)
else:
    st.info("Nenhum provento encontrado na B3 para este ativo.")
